...
# SNS Topics
USER_ACCOUNT_TOPIC_ARN = "arn:aws:sns:us-east-1:060795938414:StockerUserAccountTopic"
TRANSACTION_TOPIC_ARN = "arn:aws:sns:us-east-1:060795938414:StockerTransactionTopic"
...
# Local development mode - defaults to True if no AWS keys found, 
# but can be forced to False on EC2 using an environment variable
LOCAL_MODE = os.environ.get('LOCAL_MODE', str(not (AWS_ACCESS_KEY and AWS_SECRET_KEY))).lower() == 'true'
...
def simulate_market_movement():
    """Randomly fluctuate local stock prices to simulate a live market."""
    if not LOCAL_MODE:
        return
    for stock in local_stocks:
        change_pct = random.uniform(-0.02, 0.02) # +/- 2%
        stock['price'] = (Decimal(str(stock['price'])) * Decimal(str(1 + change_pct))).quantize(Decimal('0.01'))
        stock['change'] = f"{'+' if change_pct >= 0 else ''}{change_pct*100:.2f}%"

def get_portfolio_summary(user_id):
    """Calculate total value and profit/loss for a user's portfolio."""
    ...
    for item in portfolio:
        ...
        current_price = Decimal(str(item['stock']['price']))
        
        total_invested += qty * avg_price
        current_value += qty * current_price
        
    profit_loss = current_value - total_invested
    pl_percent = (profit_loss / total_invested * 100) if total_invested > 0 else Decimal('0.00')
